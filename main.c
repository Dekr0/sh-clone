...
int main () {
 ...
 /* --------- Variables Initialization --------- */
    ...
    int numTasks; // number of tasks sh-clone has (limit 32)
    pid_t parent_pid; // pid of sh-clone
    ...
    struct TaskDB taskList[NTASK]; // information of all tasks under sh-clone 
 ...
 /* --------- Main Loop --------- */
    ...
    while( scanf("%[^\n]", inStr) != EOF) {
        ...
        if ( cmdIndex == -1 ) {
            warning( "sh-clone: %s: command not found\n" );
            printf( "sh-clone [%d]: ", parent_pid );
            memset( inStr, 0, sizeof(inStr) );
            memset( tokens, 0, sizeof(tokens) );
            continue;
